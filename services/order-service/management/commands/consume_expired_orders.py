        ...
        try:
            ...
            if order.status == Order.Status.PENDING:
                ...
                for item in order.items.all():
                    try:
                        requests.post(
                            f'{PRODUCT_SERVICE_URL}/internal/products/{item.product_id}/restore-stock/',
                            json={'quantity': item.quantity},
                            timeout=5,
                        )
                    except Exception as e:
                        logger.error(f'[DLX] Stock restore failed for {item.product_id}: {e}')
                ...
                logger.info(f'[DLX] Order {order_id} already in status {order.status} — skip')
